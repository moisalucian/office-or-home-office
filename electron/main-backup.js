...
if (app.isPackaged) {
  ...
  autoUpdater.logger = console; // Enable detailed logging
  ...
  autoUpdater.on('error', (err) => {
    ...
    console.error('[AutoUpdater] Error details:', err.message);
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    {
      ...
      click: () => {
        ...
        if (app.isPackaged) {
          autoUpdater.checkForUpdates(); // Use checkForUpdates instead of checkForUpdatesAndNotify for manual checks
        } else {
...
app.whenReady().then(async () => {
  ...
  if (app.isPackaged) {
    setTimeout(() => {
      console.log('[AutoUpdater] Starting delayed update check...');
